...
def main(video):
    ...
    if video.get_is_short():
        for image_description in video.get_timeline_description():
            generate_image(image_description, fr'{video.get_path()}\background\img_{index}.png', width=720, height=1280)
            print(fr'IMAGE {index} / {len(video.get_timeline_description())} GENERATED')
            index += 1
    else:
        for image_description in video.get_timeline_description():
            generate_image(image_description, fr'{video.get_path()}\background\img_{index}.png', width=1280, height=720)
            print(fr'IMAGE {index} / {len(video.get_timeline_description())} GENERATED')
            index += 1
